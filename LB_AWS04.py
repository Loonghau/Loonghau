...
def GetMac():
    global ret
    node = uuid.getnode()
    ret = uuid.UUID(int=node).hex[-12:].upper()
    return ret
...
if mac == mac1:
    class Pyqt5_Serial(QtWidgets.QWidget, Ui_Form):
        def __init__(self):
            ...
            self.setupUi(self)
            self.setWindowTitle("全自动恒温恒湿称重系统V2.0")
        ...
        def run(self):
            ...
            for i in range(1, 15):
                ...
                if zmc.get_input(10) == 0:
                    ...
                    y = serial.Serial('com11', 115200, timeout=1)  # 扫码
                    ...
                    z = serial.Serial('com7', 9600, timeout=1)  # 天平
            ...
            for i in range(1, 15):
                ...
                if zmc.get_input(10) == 0:
                    ...
                    z = serial.Serial('com7', 9600, timeout=1)
                    myinput = bytes([0X53, 0X30, 0X31, 0X39, 0X39, 0X41])  # 需要发送的十六进制数据
                    th.write(myinput)  # 用write函数向串口发送数据
                    myinput1 = bytes([0X53, 0X0A, 0X0D])  # 需要发送"S"
                    z.write(myinput1)  # 用write函数向串口发送数据
    ...
    class Thread_4(QThread):  # 天平置零
        ...
        def run(self):
            ...
            zmc.set_axis_units()
            zmc.set_op(7, 0)
            x = serial.Serial('com7', 9600, timeout=1)
    ...
    class Thread_7(QThread):  # 手动扫码
        ...
        def run(self):
            y = serial.Serial('com11', 115200, timeout=1)
            myinput = bytes([0X16, 0X54, 0X0D])  # 需要发送"S"
            y.write(myinput)  # 用write函数向串口发送数据
            try:
                myout = y.read(13)  # 提取接收缓冲区中的前7个字节数
    ...
    class MyThread(QThread):  # 实时显示温湿度及天平示数
        ...
        def run(self):
            for i in range(10000):
                ...
                # y = serial.Serial('com7', 9600, timeout=1)
                # myinput1 = bytes([0X53, 0X0A, 0X0D])  # 需要发送"S"
