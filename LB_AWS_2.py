    ...
    def run(self):
        ...
        for i in range(1, 15):
            ...
            if zmc.get_input(10) == 0:
                ...
                z = serial.Serial('com9', 9600, timeout=1)  # 天平
                ...
                ws.append(list)
                wb.save(excelName1)
                new_name = str(now_time_1) + str(csnum) + ".xls"
        ...
        for i in range(1, 15):
            ...
            if zmc.get_input(10) == 0:
                ...
                z = serial.Serial('com9', 9600, timeout=1)
                myinput = bytes([0X53, 0X30, 0X31, 0X39, 0X39, 0X41])  # 需要发送的十六进制数据
                th.write(myinput)  # 用write函数向串口发送数据
                myinput1 = bytes([0X53, 0X0A, 0X0D])  # 需要发送"S"
                z.write(myinput1)  # 用write函数向串口发送数据
...
class Thread_4(QThread):  # 天平置零
    ...
    def run(self):
        ...
        zmc.set_axis_units()
        zmc.set_op(7, 0)
        x = serial.Serial('com9', 9600, timeout=1)
...
class MyThread(QThread):  # 实时显示温湿度及天平示数
    ...
    def run(self):
        for i in range(10000):
            ...
            # y = serial.Serial('com9', 9600, timeout=1)
            # myinput1 = bytes([0X53, 0X0A, 0X0D])  # 需要发送"S"
